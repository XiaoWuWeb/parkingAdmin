$(function(){
	function __oneManager(){
		...
		common.ajax('/fault/feedinfo',oDetail,function(res){
			console.log(res);
	...
	function __twoManager(){
		...
		common.ajax('/fault/feedinfo',oData,function(res){
			console.log(res);
			console.log(res.map.length);
			if (res.map.length > 0) {
				var detail=res;
	...
	$('body').on('click','.detail_btn',function(){
		// 取id，存页头或者cookie
		window.location.href = 'fixFeedbackDetail.html'
	...
	function __removeParking(data){
		...
		function(){
			var oData = data;
			console.log(oData)
			common.ajax('/fault/delfeedback',oData,function(res){
				if (res.success) {
					swal({

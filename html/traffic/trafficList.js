$(function(){
	common.ajax('/traffic/Violations','',function(res){
		console.log(res);
		if(res.map.length > 0){
			var detail = res;
	...
	function __pagesNext(n){
		...
		if (oCarAddress.length > 0) {
			oData.appellation = oCarAddress;
		}
		if (oCarNum.length > 0) {
			oData.IllegallyTime = oCarNum;
		}
		__pageAll(oData)
	}
	...
	function __pageAll(data){
		var oData = data;
		common.ajax('/traffic/Violationss',oData,function(res){
			console.log(res);
			if(res.map.length > 0){
				var detail = res;
				var model=$("#model").html();//获取模板有<%
	...
	$('body').on('click','.search-btn',function(e){
		...
		// oData.parkingNo = oCarNum;
		if (oName.length > 0) {
			oData.appellation = oName;
		}
		if (oTime.length > 0) {
			oData.IllegallyTime = oTime;
		}
		common.ajax('/traffic/Violationss',oData,function(res){
			console.log(res);
			if(res.map.length > 0){

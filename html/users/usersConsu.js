$(function(){
	common.ajax('/appuser/selectRecords','',function(res){
		console.log(res);
		if(res.map.length > 0){
			var detail = res;
	...
	function __pagesNext(n){
		...
		if (oArea.length > 0) {
			oData.areaName = oArea;
		}
		__pageAll(oData)
	}
	function __pageAll(data){
		var oData = data;
		common.ajax('/appuser/selectRecords',oData,function(res){
			if(res.map.length > 0){
				var detail = res;
				var model=$("#model").html();//获取模板有<%
				var template=_.template(model)(detail);//把数据填充到模板里面并去掉<%
	...
	$('body').on('click','.search-btn',function(e){
		...
		common.ajax('/appuser/selectRecordss',oData,function(res){
			console.log(res);
			if(res.map.length > 0){
				var detail = res;
				var model=$("#model").html();//获取模板有<%

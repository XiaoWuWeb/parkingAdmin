$(function(){
	common.ajax('/admin/selectWithdrawals','',function(res){
		console.log(res);
		if (res.map.length > 0) {
			var detail=res;
			...
			var oHtml = '<p style="width:1638px;height:100px;font-size:16px;text-align:center;line-height:100px;">当前暂时没有提现申请记录!</p>'
	...
	function __pageAll(data){
		var oData = data;
		common.ajax('/admin/selectWithdrawals',oData,function(res){
			if (res.map.length > 0) {
				var detail=res;
				var model=$("#model").html();//获取模板有<%
				var template=_.template(model)(detail);//把数据填充到模板里面并去掉<%
	...
	$('body').on('click','.detail',function(){
		var oId = $(this).find('span.id').text();
		window.location.href = 'ownersMoneyDetail.html?id=' + oId;
	});
